...
void QUINTO_init() {
	...
	*/
	QUINTO_data_spec=INIT_DATA_SPEC(DATA_TYPE_E,DATA_UNIT_E,range,EXP_TIME_MS,ACCURACY);
	/*
	...
	*/
	...
	QUINTO_data_value_spec=INIT_VALUE_SPEC(DATA_VALUE_TYPE_E,range,data_dimensions);
	if (data_dimensions>0) {
		/*
		Definire il numero di elementi massimi per ogni dimensione 
		QUINTO_data_value_spec.max_elements[0]=_MAX_ELEMENTS_
		QUINTO_data_value_spec.max_elements[1]=_MAX_ELEMENTS_
		..
		..
		QUINTO_data_value_spec.max_elements[data_dimensions-1]=_MAX_ELEMENTS_
		*/
	}
	/*
	Il dato
	DATA_TYPE_E è un enum definito in data.h va aggiunto un campo tipicamente corrispondente al nome del modulo
	*/
	QUINTO_data=INIT_DATA(DATA_TYPE_E,BASEMODULE_data_value_spec);
    SUB(QUINTO_data_spec.type);
	SET_S(QUINTO_data_spec);
}
...
void QUINTO_run() {
  ...
  */
  QUINTO_data.value.DATA_VALUE_U
  SETDATATIMESTAMP(&QUINTO_data);
  SET_D(QUINTO_data);
}

void QUINTO_close() {
  PRINTA("QUINTO_CLOSE");
  UNSUB(QUINTO_data_spec.type);
}
